...
async function fetchkupotKlaliB() {
  try {
    ...
    const response = await fetch('bituachKlali.xml');
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const row = rows[i];
      const shemKovetz="dataJasonB.json";
      const apiName="bituach"; 
    ...
    console.error("Error in fetchkupotKlaliB:", error);
    return [];
  }
}
...
async function makejasonB() {
        ...
        console.log("הקובץ נוצר בהצלחה!");
         localStorage.setItem("downloadIndex", 2);
